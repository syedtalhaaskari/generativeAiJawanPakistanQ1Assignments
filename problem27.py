...
def calc_dst_end(year: datetime, tz):
    tz = pytz.timezone(tz)
            ...
            while True:
                ...
                if us_time.dst().total_seconds() == 0:
                    while True:
                        ...
                        us_time = tz.localize(us_time)
                        if us_time.dst().total_seconds() > 0:
                            return us_time + timedelta(hours=1)
                        
year = 2022
tz = "US/Pacific"
dst_start = calc_dst_end(year, tz)

print(f"Daylight Saving Time for {tz} for the year {year} will end at {dst_start}")