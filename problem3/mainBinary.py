...
def add_password():
    try:
        ...
        with open('passwords.pkl', 'ab') as file:
            data = pickle.dumps(f"{answers['website']},{answers['username']},{answers['password']}\n")
            file.write(data)
...
def delete_password():
    try:
        ...
        with open('passwords.pkl', 'rb') as file:
            ...
            if next_read == b'':
                file.close()
            data = pickle.loads(next_read).strip()
            while data:
                lines.append(data)
                next_read = file.read()
                if next_read == b'':
