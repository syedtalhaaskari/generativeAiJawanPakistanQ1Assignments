...
class Laptop(Item):
    def __init__(self, name: str, price: float, quantity: int, gpu: str, port_count: int):
...
for (i, (name, price, quantity)) in enumerate(items):
    separator()
    if name == 'Laptop':
        items_obj[i] = Laptop(name, price, quantity, 'AMD Radeon 7600xt', 3)
    else: 
    ...
    if name == 'Laptop':
        print(f"GPU: {items_obj[i].gpu}")
        print(f"Port Count: {items_obj[i].port_count}")        
    print(f"Discount/Item: {items_obj[i].apply_discount(0.3)}")
    print(f"Total Price: {items_obj[i].calculate_total_price(0.3)}")
